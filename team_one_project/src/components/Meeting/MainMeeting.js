...
function getCurrentWeekDateRange(referenceDate) {
  ...
  const startDateString = startOfWeek.toLocaleDateString(undefined, formatOptions);
  ...
  return (
    <div className="container">
      <div className="calendar">
        ...
        <button className="button-prev" onClick={moveToPreviousWeek}>&lt; Previous Week</button>
          <h2>{dateRange}</h2>
          <button className="button-next" onClick={moveToNextWeek}>Next Week &gt;</button>
        </div>
        <div className="grid">
            ...
            <div className="row" key={time}>
              ...
              {days.map(day => (
                <button 
                  ...
                  className={`cell timeSlot ${selectedSlots.includes(`${day}-${time}`) ? 'selected' : ''}`} 
                  key={day}
        ...
        <div className="selectedSlots">
          ...
          <ul>
            {selectedSlots.map(slot => <li key={slot}>{slot.replace('-', ' at ')}</li>)}
          </ul>
        </div>
        <div className="actions">
        <button className="button-clear" onClick={() => setSelectedSlots([])}>Clear Selection</button>
        <button className="button-confirm" onClick={() => console.log("Confirmed slots:", selectedSlots)}>Confirm Selection</button>
        </div>
      </div>
    </div>
  );
